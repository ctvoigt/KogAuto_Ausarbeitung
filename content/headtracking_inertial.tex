...
\todo[inline]{Ich bitte den Ersteller des TF-Baums sich dieser Grafik anzunehmen, der sieht sehr proffesionell aus, sowas in abgespeckter Form brauchen wir hier auch, damit wir uns in den nachfolgenden Sections drauf beziehen können, bitte die Textreferenzen in \ref{headtracking_imu_subsec} dazu anpassen.}
...
\todo[inline]{Brillen-KS taucht hier zum ersten Mal auf, das sollte noch eingeführt werden.}
...
Sobald das Erdmagnetfeld nicht am Äquator gemessen wird, sondern im Falle des Praktikums bei etwa $49^\circ$ geographischer Breite, muss der Inklinationswinkel bei der Messung mit beachtet werden.
Dieser wird in Abbildung \ref{fig:mag_mapping} mit I beschrieben.
...
\end{equation}
\todo{(7) hängt da so schwul drunter}
\todo{Ohne das Wort ``mit'' sieht es besser aus'}
...
Allerdings konnten durch mehrere Testläufe im Fahrzeug keine größeren Störeffekte gemessen werden.\todo{Zu "erzählerisch"?}\
